...
short prc_enc(short* pcm)
{
 ...
 if(prc_vton)
 {
  if(prc_vtcnt<AMR_SPEECH) 
	{
		memcpy(prc_tbuf[prc_in_tptr], speech_tbl[prc_vtcnt], 12);
		i=1;
	}
	else
	{
		memset(prc_tbuf[prc_in_tptr], 0, 12);
		prc_tbuf[prc_in_tptr][0]=1;
		i=0;
	}
	prc_vtcnt++;
	if(prc_vtcnt>=(AMR_SPEECH+VPAUSE)) prc_vtcnt=0;
 }	 
#endif	
...
//check we haven't samples for output frame 
 ...
 play_end:
 for(i=0;i<FRAMELENGTH;i++) pcm[i]^=0x8000; //convert short to unsigned short for DAC 
